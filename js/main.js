...
function spawnExplosion(x, y, color) { for(let i = 0; i < 25 && particles.length < MAX_PARTICLES; i++) particles.push({ x: x, y: y, vx: (Math.random() - 0.5) * 20, vy: (Math.random() - 0.5) * 20, l[...]
...
function startGame(seed, solo = false) {
    ...
    if (isSoloMode || !conn || !conn.open) { document.getElementById('opp-score-container').style.display = 'none'; document.getElementById('heart-display').style.display = 'block'; document.getEl[...]
    else { document.getElementById('opp-score-container').style.display = 'block'; document.getElementById('heart-display').style.display = 'none'; document.getElementById('rocket-counter').style.[...]
    initGame(seed);
}
...
function initGame(seed) {
    ...
    player = { x: canvas.width / 2 - 15, y: canvas.height / 2, targetX: canvas.width / 2 - 15, w: 30, h: 30, vy: 0, jumpPower: baseJump, colorStage: 0, squash: 1.0, angle: 0, flipAngle: 0, flipDir[...]
    
    document.getElementById('mic-counter').innerText = `🎤: 0/3`; document.getElementById('heart-count').innerText = player.hearts; document.getElementById('rocket-count').innerText = player.roc[...]
    platforms = [{ x: canvas.width/2 - PLATFORM_W/2, y: canvas.height - 100, w: PLATFORM_W, h: PLATFORM_H, visited: true, dx: 0, hp: 6, hasRevive: false, hasShrink: false, hasSnow: false, hasWeb: [...]
...
function gameOver() {
    ...
    if (score > myHighScore) { myHighScore = score; localStorage.setItem('neonJumperHighScore', myHighScore); document.getElementById('lobby-highscore').innerText = myHighScore; submitScore(myNam[...]
...
document.getElementById('connect-btn').onclick = async () => {
    ...
    if(targetId) { await requestMic(); document.getElementById('connect-btn').innerText = "Đang kết nối..."; conn = peer.connect(targetId); setupConnection(); conn.on('open', () => { documen[...]
};
document.getElementById('solo-btn').onclick = async () => { saveProfile(); await requestMic(); startCountdown(Math.random(), true); };
document.getElementById('restart-btn').addEventListener('click', () => { let seed = Math.floor(Math.random() * 99999); if (conn && conn.open) { conn.send({ type: 'start', seed: seed }); startCoun[...]

const gyroBtn = document.getElementById('gyro-btn');
function handleOrientation(e) { let gamma = e.gamma; if (gamma > 45) gamma = 45; if (gamma < -45) gamma = -45; gyroGamma = gamma; }
function enableGyro() { useGyro = true; gyroBtn.classList.add('gyro-active'); window.addEventListener('deviceorientation', handleOrientation); showToast("📱 Bật cảm biến lắc: ĐIỂM G[...]
gyroBtn.onclick = () => {
    if (useGyro) { useGyro = false; gyroBtn.classList.remove('gyro-active'); window.removeEventListener('deviceorientation', handleOrientation); player.targetX = player.x; showToast("📱 Đã t[...]
    else { if (typeof DeviceOrientationEvent !== 'undefined' && typeof DeviceOrientationEvent.requestPermission === 'function') { DeviceOrientationEvent.requestPermission().then(state => { if (st[...]
};
