...
function showToast(text, color) {
    ...
    toast.style.transform = 'translateX(-50%) translatefunction resizeCanvas() { canvas.width = window.innerWidth; canvas.height = window.innerHeight; }
resizeCanvas();
window.addEventListener('resize', resizeCanvas);
...
// --- HÀM XỬ LÝ HỒI SINH TỰ ĐỘNG BẮN TƠ CỨU NGUY ---
function triggerPlayerRevive(p) {
    p.isDead = false;
    p.vy = -12; // Hất nhẹ nhân vật lên cao khi hồi sinh
    
    // Tự động tìm khối gần nhất ở phía trên để bắn tơ cứu nguy
    let bestPlatform = null;
    let minDst = Infinity;
